...
class binary_classifier(object):
    ...
    def fit(self, X, y, step_size, initial_weights = None,\
            maximum_iterations = 1000, verbose = False):

        '''
        A method which carries out gradient descent of logistic regression
        Arguments:
        X - feature matrix H of shape (number of examples, number of features)
        y - vector of labels of shape (number of examples,)
        step_size - step size/learning rate of gradient descent
        initial_weights - vector of initial weights
        maximum_iterations - maximum of number of iterations
        verbose - boolean variable which indicates if verbose be printed out during each iteration
        '''

        # if no initial weight is given then the initial weight is a zero vector
        ...
        # iterate for maximum number of times
        for iter in range(maximum_iterations):

            # compute gradient descent step to update the weights
            weights = self.gradient_descent_step(X, y, weights, step_size)

            num = 50
            step = maximum_iterations//num
            # print verbose if the user wants
            if  (iter%step == 0 and iter != 0) and verbose:
                logistic_cost = self.compute_logistic_cost(X, weights, y)
                print('iteration %*d: logistic cost = %.8f' % \
                    (int(np.ceil(np.log10(maximum_iterations))), iter, logistic_cost))

            # track logistic regression cost at each iteration if the user wants
            if self.track_logistic_cost:
                self.log_of_logistic_cost.append(self.compute_logistic_cost(X, weights, y))
